...
describe("SapAICorePlugin", () => {
  ...
  it.effect("preserves existing AICORE_SERVICE_KEY over serviceKey option", () =>
    withEnv({ AICORE_SERVICE_KEY: "env-service-key", AICORE_DEPLOYMENT_ID: "deployment", AICORE_RESOURCE_GROUP: "resource-group" }, () =>
      Effect.gen(function* () {
        const plugin = yield* PluginV2.Service
        yield* plugin.add(pluginWithNpm)
        const sdk = yield* plugin.trigger("aisdk.sdk", {
          model: model("sap-ai-core", "sap-model"),
          package: fixtureProvider,
          options: { serviceKey: "option-service-key" },
        })
  ...
  it.effect("uses the callable SDK for language selection", () =>
    Effect.gen(function* () {
      ...
      const language = yield* plugin.trigger("aisdk.language", { model: model("sap-ai-core", "sap-model"), sdk, options: {} })
  ...
  it.effect("ignores non-SAP AI Core providers", () =>
    withEnv({ AICORE_SERVICE_KEY: undefined, AICORE_DEPLOYMENT_ID: "deployment", AICORE_RESOURCE_GROUP: "resource-group" }, () =>
      Effect.gen(function* () {
        ...
        const language = yield* plugin.trigger("aisdk.language", {
          model: model("openai", "sap-model"),
          sdk: () => {
