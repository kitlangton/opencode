...
import { AuthV2 } from "../../../src/v2/auth"
import { ModelV2 } from "../../../src/v2/model"
import { PluginV2 } from "../../../src/v2/plugin"
import { AuthPlugin } from "../../../src/v2/plugin/auth"
...
function cloudflareLanguage(sdk: unknown, modelID = "@cf/model") {
  return (sdk as { languageModel: (id: string) => { config: CloudflareConfig; provider: string } }).languageModel(modelID)
}
    ...
    withEnv({ CLOUDFLARE_ACCOUNT_ID: "acct", CLOUDFLARE_API_KEY: "env-key" }, () =>
      Effect.gen(function* () {
        ...
        const result = yield* plugin.trigger("aisdk.sdk", {
          ...
          package: "@ai-sdk/openai-compatible",
          options: { apiKey: "auth-key", baseURL: "https://proxy.example/v1", headers: { custom: "header" } },
        })
    ...
    withEnv({ CLOUDFLARE_ACCOUNT_ID: "acct", CLOUDFLARE_API_KEY: "key" }, () =>
      Effect.gen(function* () {
        ...
        const result = yield* plugin.trigger("aisdk.sdk", {
          ...
          package: "@ai-sdk/openai-compatible",
          options: { baseURL: "https://api.cloudflare.com/client/v4/accounts/${CLOUDFLARE_ACCOUNT_ID}/ai/v1" },
        })
        expect(cloudflareURL(result.sdk)).toBe("https://api.cloudflare.com/client/v4/accounts/acct/ai/v1/chat/completions")
      }),
    ),
  )
