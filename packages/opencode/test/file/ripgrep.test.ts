...
const it = testEffect(Layer.mergeAll(Ripgrep.defaultLayer, AppFileSystem.defaultLayer, CrossSpawnSpawner.defaultLayer))

const tmpdir = <E, R>(init?: (dir: string) => Effect.Effect<void, E, R>) =>
  tmpdirScoped().pipe(Effect.tap((dir) => init?.(dir) ?? Effect.void))

const write = (file: string, data: string) => AppFileSystem.Service.use((fs) => fs.writeWithDirs(file, data))
